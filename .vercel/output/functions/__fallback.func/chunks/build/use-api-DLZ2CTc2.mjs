import { l as useToast, c as useSession, i as fetchDefaults, j as useAsyncData, k as useRequestFetch } from './server.mjs';
...
function useApi(request, options) {
  const toast = useToast();
  const session = useSession();
  return useFetch(request, {
    onRequest({ options: options2 }) {
      if (session.token) {
        options2.headers.set("Authorization", `Bearer ${session.token}`);
      }
    },
...
export { useApi as u };
//# sourceMappingURL=use-api-DLZ2CTc2.mjs.map
